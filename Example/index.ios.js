...
class SampleApp extends React.Component {
  ...
  render() {
    ...
    return (
      <TabBarIOS tintColor="white" barTintColor="slategray">
        <TabBarIOS.Item
          title="Fire" icon={{uri: "mysprite", scale: 3}}
          selected={this.state.selectedTab === 'fireTab'}
          onPress={() => {
            this.setState({
              selectedTab: 'fireTab',
            });
        ...
        <TabBarIOS.Item
          title="Stars"
          selected={this.state.selectedTab === 'starsTab'}
          onPress={() => {
            this.setState({
              selectedTab: 'starsTab',
            });
          }}>
          {
            <View style={{flex:1, backgroundColor:'black'}}>
            <ParticleView name={"emitterLayer"} style={{flex:1,backgroundColor:'transparent'}}
              ...
              <ParticleCell {...starsParticleProps}>
                <Image source={{uri:"mysprite"}} style={{top:0, left:0, width:32, height:32}}/>
              </ParticleCell>
            </ParticleView>
          </View>
          }
